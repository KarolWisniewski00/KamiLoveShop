...
<div class="row mb-3">
    <div class="col-12">
        <div class="d-flex flex-row justify-content-start align-items-center">
            ...
            <span class="badge rounded-pill bg-primary">Wybrano: Edytowanie podkategorii</span>
        </div>
    </div>
    <hr>
    ...
    <div class="col-12">
        <form method="POST" action="{{route('admin.subcategory.update', $subcat->id)}}">
            ...
            <div class="form-floating my-3 w-100">
                <input type="text" class="form-control" id="name" value="{{$subcat->name}}" name="name" required>
                <label for="name">Nazwa</label>
                <span class="text-danger">@error('name') {{$message}} @enderror</span>
            </div>

            <div class="form-floating my-3 w-100">
                <input type="text" class="form-control" id="plural" value="{{$subcat->plural}}" name="plural" required>
                <label for="plural">L.mnoga</label>
                <span class="text-danger">@error('plural') {{$message}} @enderror</span>
            </div>

            <div class="form-floating my-3 w-100">
                <input type="text" class="form-control" id="url" value="{{$subcat->url}}" name="url" required>
            ...
            <div class="input-group my-3 w-100">
                ...
                <select class="form-select" id="category_id" name="category_id" required>
                    <option value="{{$subcat->category_id}}">{{$subcat->category->plural}}</option>
                    @foreach ($cat as $c)
            ...
            <div class="form-floating my-3 w-100">
                <input type="text" class="form-control" id="order" value="{{$subcat->order}}" name="order" required>
