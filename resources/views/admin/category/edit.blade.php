...
<div class="row mb-3">
    <div class="col-12">
        <div class="d-flex flex-row justify-content-start align-items-center">
            ...
            <span class="badge rounded-pill bg-primary">Wybrano: Edytowanie kategorii</span>
        </div>
    </div>
    <hr>
    ...
    <div class="col-12">
        <form method="POST" action="{{route('admin.category.update', $cat->id)}}">
            ...
            <div class="form-floating my-3 w-100">
                <input type="text" class="form-control" id="name" value="{{$cat->name}}" name="name" required>
                <label for="name">Nazwa</label>
                <span class="text-danger">@error('name') {{$message}} @enderror</span>
            </div>

            <div class="form-floating my-3 w-100">
                <input type="text" class="form-control" id="plural" value="{{$cat->plural}}" name="plural" required>
                <label for="plural">L.mnoga</label>
                <span class="text-danger">@error('plural') {{$message}} @enderror</span>
            </div>

            <div class="form-floating my-3 w-100">
                <input type="text" class="form-control" id="url" value="{{$cat->url}}" name="url" required>
                <label for="url">URL</label>
                <span class="text-danger">@error('url') {{$message}} @enderror</span>
            </div>

            <div class="form-floating my-3 w-100">
                <input type="text" class="form-control" id="order" value="{{$cat->order}}" name="order" required>
            ...
            <div class="row border rounded p-4 mx-1">
                <div class="col-12 col-md-6">
                    ...
                    <div class="d-flex flex-column justify-content-center align-items-center border">
                        <img src="{{asset('photos/'.$cat->photo)}}" class="img-fluid bd-placeholder-img p-2" onerror="this.onerror=null; this.src=`{{ asset('svg/photos.svg') }}`;">
                    </div>
                </div>
                <div class="col-12 col-md-6">
